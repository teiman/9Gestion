...
function EnviarMensaje($IdAutor,$titulo,$texto,$modo = "Normal",$toLocal=0,$toUser=0){
	$titulo = CleanText($titulo);
	$texto	= CleanText($texto);
	
	$sql = "INSERT INTO ges_mensajes (Titulo, Texto, IdAutor, IdLocalRestriccion, IdUsuarioRestriccion, Status, Fecha) 
		VALUES 
		('$titulo', '$texto', '$IdAutor', '$toLocal', '$toUser', '$modo', NOW())";
...
switch($modo){
	...
	case "CargarMensaje":
		...
		break;
	case "leernuevos":
		$IdUltimo = CleanID($_GET["IdUltimo"]);
	case "hoy":
		if (!$IdUltimo)
			$IdUltimo = 0;
	...
	case "avisonotaprivada":	
		...
		if ( EnviarMensaje($origen,$titulo,$texto,"Normal",$destino) ) {
			echo "OK";
		} else {
			echo "ERROR";
		}		
